...
contract('Membership', function(accounts) {
    ...
    it("Should refund amout to user that did not pass KYC/AML", async function () {
        balance1 = await (web3.fromWei(web3.eth.getBalance(accounts[1]), 'ether').toFixed(0));
        await membership.refund(accounts[1], 1, {from: accounts[5]});
        let newbal = eval(await (web3.fromWei(web3.eth.getBalance(accounts[1]), 'ether').toFixed(0)));
    ...
    it("Should allow owner to withdraw funds to address specified", async function () {
        ...
        await membership.withdraw(accounts[7], 4, {from: accounts[5]});
        let newbal2 = eval(await web3.fromWei(web3.eth.getBalance(accounts[7]), 'ether').toFixed(0));
